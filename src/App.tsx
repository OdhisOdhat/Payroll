...
const App = () => {
  ...
  const [payrollSortField, setPayrollSortField] = useState('processedAt');
  const [payrollSortDir, setPayrollSortDir] = useState('desc');
  ...
  const getAiTaxAdvice = async (emp) => {
    setLoadingAi(true);
    setAiInsight('');
    try {
  ...
  const handleGetTaxOptimization = async (emp) => {
  ...
  const handleGenerateP9Breakdown = async (emp) => {
  ...
  const handleSaveEmployee = async (e) => {
    ...
    const designation = (formData.get('designation') || 'Staff').trim();
    const basicSalary = parseFloat(formData.get('basicSalary')) || 0;
    const benefits = parseFloat(formData.get('benefits')) || 0;
    const totalLeaveDays = parseInt(formData.get('totalLeaveDays')) || 21;
    const remainingLeaveDays = parseInt(formData.get('remainingLeaveDays')) || totalLeaveDays;
    ...
    const employeeData = {
      ...
      nhifNumber: formData.get('nhifNumber') || '',
    ...
    try {
      ...
      const url = isUpdate
        ? `http://localhost:4000/api/employees/${editingEmployee.id}`
      ...
      if (isUpdate) {
        setEmployees(prev => prev.map(emp => emp.id === savedFromDb.id ? savedFromDb : emp));
      } else {
        setEmployees(prev => [...prev, savedFromDb]);
      }
      ...
      if (user) {
        await apiService.saveAuditLog({
          ...
          performedBy: `${user.firstName} ${user.lastName}`,
          userRole: user.role,
          action: isUpdate ? 'DB_UPDATE' : 'DB_INSERT',
          details: `Persistence successful for ${employeeData.name} in PostgreSQL.`,
          timestamp: new Date().toISOString()
        });
      }
  ...
  const handleTerminateEmployee = async (employeeId) => {
    ...
    try {
      ...
      alert(`Failed to terminate employee: ${err.message || 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLeaveStatusUpdate = async (id, status, employeeId, startDate, endDate) => {
    ...
    try {
      ...
      if (status === 'approved') {
        setEmployees(prev => prev.map(e => 
          e.id === employeeId 
            ? { ...e, remainingLeaveDays: Math.max(0, e.remainingLeaveDays - diffDays) } 
            : e
        ));
      }
  ...
  const processPayrollRun = async () => {
    ...
    const newRecords = accessibleEmployees.map(emp => ({
  ...
  const handleFileChange = async (event) => {
    ...
    reader.onload = async (e) => {
      const importedData = parseEmployeeCSV(e.target?.result);
      if (importedData.length === 0) return alert("No valid data found in CSV file.");
      ...
      try {
        ...
        setEmployees(Array.isArray(freshEmployees) ? freshEmployees : []);
  ...
  const handleExcelImport = async (event) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!arrayBuffer) throw new Error("No data loaded from file");
        ...
        worksheet.eachRow({ includeEmpty: true }, (row, rowNumber) => {
          ...
          if (rowNumber === 1) {
            headers = rowValues.slice(1).map(h => 
        ...
        const imported = jsonData.map(obj => {
          return {
            ...
            kraPin: obj.krapin || obj['kra pin'] || obj.pin || '',
            nssfNumber: obj.nssf || obj.nssfnumber || obj['nssf number'] || '',
            nhifNumber: obj.nhif || obj.sha || obj['sha number'] || obj.nhifnumber || '',
            designation: obj.designation || obj.position || obj.title || obj.role || 'Staff',
            basicSalary: Number(obj.basicsalary || obj['basic salary'] || obj.salary || 0),
            benefits: Number(obj.benefits || obj.allowances || obj['other benefits'] || 0),
            totalLeaveDays: Number(obj.totalleavedays || obj['total leave'] || obj.leavedays || 21),
            remainingLeaveDays: Number(obj.remainingleavedays || obj['remaining leave'] || obj.leavebalance || 21),
            joinedDate: obj.joineddate || obj['date joined'] || obj.hiredate || new Date().toISOString(),
          };
        }).filter(e => {
        ...
        if (imported.length === 0) {
          alert(
            ...
            "The file was read successfully, but no rows had all required fields:\n" +
            "• First Name\n• Last Name\n• Payroll Number\n" +
            "Please check your column headers and data."
          );
          return;
        }
        ...
        for (const data of imported) {
          try {
            ...
            console.warn("Failed to save employee:", err);
          }
        }
        ...
        alert(
          `Successfully imported ${successCount} of ${imported.length} employees.\n` +
          `If employees don't appear, try refreshing the page.`
        );
      } catch (err) {
        console.error("Excel import error:", err);
        alert("Failed to read Excel file. Please ensure it's a valid .xlsx file.");
      } finally {
        setIsLoading(false);
  ...
  // Print functions for separate documents
  const handlePrintPayslip = () => {
    if (!selectedEmployee) {
      alert("Please select an employee first to generate the payslip.");
      return;
    }
    ...
    printWindow.document.write(`
      ...
      <head>
        ...
        <title>Payslip - ${selectedEmployee.firstName} ${selectedEmployee.lastName}</title>
        <style>
          body {
            margin: 0;
            padding: 20px;
    ...
    const payslipElement = document.querySelector('#payslip-print-content');
    if (payslipElement) {
      const cloned = payslipElement.cloneNode(true);
      cloned.querySelectorAll('.no-print, button, input, select').forEach(el => el.remove());
      printWindow.document.querySelector('.print-content').appendChild(cloned);
    } else {
      printWindow.document.body.innerHTML += `
        <div style="text-align:center; padding:50px; color:#666;">
          <h2>Payslip Content Not Found</h2>
          <p>Please ensure an employee is selected and try again.</p>
        </div>
      `;
    }
  ...
  const handlePrintP9Form = () => {
    if (!selectedEmployee) {
      alert("Please select an employee first to generate the P9 form.");
      return;
    }
    ...
    printWindow.document.write(`
      ...
      <head>
        <meta charset="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <title>P9 Form - ${selectedEmployee.firstName} ${selectedEmployee.lastName}</title>
        <style>
          body {
            margin: 0;
            padding: 20px;
    ...
    const p9Element = document.querySelector('#p9-print-content');
    if (p9Element) {
      const cloned = p9Element.cloneNode(true);
      cloned.querySelectorAll('.no-print, button, input, select').forEach(el => el.remove());
      printWindow.document.querySelector('.print-content').appendChild(cloned);
    } else {
      printWindow.document.body.innerHTML += `
        <div style="text-align:center; padding:50px; color:#666;">
          <h2>P9 Form Content Not Found</h2>
          <p>Please ensure an employee is selected and try again.</p>
        </div>
      `;
    }
        ...
        <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-700">
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-3xl custom-theme-bg shadow-2xl mb-6">
              {brandSettings.logoUrl ? (
                <img src={brandSettings.logoUrl} className="w-10 h-10 object-contain" alt="Logo" />
              ) : (
          ...
          <div className="bg-slate-900/50 border border-slate-800 backdrop-blur-xl p-6 md:p-10 rounded-[30px] md:rounded-[40px] shadow-2xl">
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-950/50 border-2 border-slate-800 rounded-2xl pl-12 pr-4 py-4 text-white outline-none focus:border-blue-500 transition-all font-bold" 
                  />
                </div>
              </div>
              <div className="space-y-2">
              ...
              <button 
                ...
                className="w-full custom-theme-bg hover:opacity-90 text-white font-black py-4 rounded-2xl transition-all disabled:opacity-50 flex items-center justify-center gap-2 uppercase tracking-widest text-sm"
              >
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[1000] lg:hidden animate-in fade-in">
          ...
          <div className="absolute inset-y-0 left-0 w-[280px] bg-slate-900 shadow-2xl animate-in slide-in-from-left duration-300">
            ...
            <button 
              onClick={() => setIsMobileMenuOpen(false)} 
              className="absolute top-6 right-[-50px] p-2 text-white bg-slate-900 rounded-full shadow-xl"
            >
              <X size={24}/>
            </button>
          </div>
        ...
        <header className="lg:hidden flex items-center justify-between p-4 bg-white border-b border-slate-200 sticky top-0 z-[100] no-print">
          <div className="flex items-center gap-3">
            <button 
              ...
              className="p-2 bg-slate-50 rounded-xl text-slate-600 active:bg-slate-100 transition-all"
            >
              <Menu size={24} />
            </button>
            <div className="font-black text-slate-800 tracking-tight text-sm truncate max-w-[150px]">
              {brandSettings.entityName}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className={`px-2 py-1 rounded-full text-[8px] font-black uppercase flex items-center gap-1 border ${
              dbStatus === 'online' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'
            }`}>
              <Zap size={8} className={dbStatus === 'online' ? 'fill-emerald-600' : ''} /> 
              {dbStatus === 'online' ? 'Online' : 'Local'}
            </div>
          </div>
        </header>
        ...
        <div className="p-4 md:p-10 max-w-7xl mx-auto w-full">
          {activeTab === 'dashboard' && (
            <div className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                <StatCard 
                  title="NSSF (Security)" 
                  value={stats.totalNssf} 
                  color="custom-theme-text" 
                  bgColor="bg-blue-50" 
                  icon={<ShieldCheck size={16}/>} 
                />
                <StatCard 
                  title="SHA (Health)" 
                  value={stats.totalSha} 
              ...
              {user.role !== 'staff' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
                  <div className="lg:col-span-2 bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 border border-slate-100 shadow-sm">
                    <div className="flex items-center justify-between mb-6 md:mb-8">
                      <h3 className="text-lg md:text-xl font-black text-slate-800 flex items-center gap-2">
                        <History className="custom-theme-text" /> Audit Ledger
                      </h3>
                      <button 
                        onClick={() => apiService.getAuditLogs().then(setAuditLogs)} 
                        className="text-[10px] font-black uppercase custom-theme-text bg-blue-50 px-3 py-2 rounded-lg"
                      >
                        Refresh
                      </button>
                    </div>
                    <div className="space-y-4">
                      {auditLogs.length > 0 ? (
                        auditLogs.slice(0, 5).map(log => (
                          <div 
                            key={log.id} 
                            className="flex gap-3 md:gap-4 p-3 md:p-4 rounded-xl md:rounded-2xl border border-slate-50 hover:bg-slate-50 transition-all"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start mb-1">
                                <div className="font-bold text-xs md:text-sm text-slate-800 truncate pr-2">
                                  {log.action}
                                </div>
                                <div className="text-[8px] md:text-[10px] text-slate-400 font-bold shrink-0">
                                  {new Date(log.timestamp).toLocaleDateString()}
                                </div>
                              </div>
                              <div className="text-[10px] md:text-xs text-slate-500 font-medium mb-1 line-clamp-1">
                                {log.details}
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="bg-slate-900 rounded-2xl md:rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-2xl hidden md:block">
                    <div className="absolute top-0 right-0 p-10 opacity-10 custom-theme-text">
              ...
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                {user.role === 'staff' && (
                  <button 
                    ...
                    className="custom-theme-bg text-white px-6 py-4 rounded-2xl flex items-center justify-center gap-2 hover:opacity-90 shadow-xl font-bold w-full md:w-auto"
                  >
                    ...
                    <table className="w-full text-left min-w-[600px]">
                      ...
                      <tbody>
                        {leaveRequests.length > 0 ? (
                          ...
                          <tr>
                            <td 
                              colSpan={(user.role === 'admin' || user.role === 'manager') ? 4 : 3} 
                    ...
                    {user.role === 'staff' && selectedEmployee ? (
                      <div className="relative space-y-4">
                        ...
                        <div className="text-4xl md:text-5xl font-black">
                          {selectedEmployee.remainingLeaveDays} <span className="text-sm text-slate-500 uppercase">Days</span>
                        </div>
                      </div>
                    ) : (
                      ...
                      <tbody>
                        {filteredEmployeesList.length > 0 ? (
                          filteredEmployeesList.map(emp => (
                            ...
                            >
                              <td className="py-4 px-6 text-xs font-black custom-theme-text">
                                {emp.payrollNumber}
                              </td>
                              <td className="py-4 px-6">
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-lg custom-theme-bg flex items-center justify-center text-white text-xs font-bold shrink-0">
                                    {emp.firstName[0]}
                                  </div>
                              ...
                              <td className="py-4 px-6">
                                <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase ${
                                  emp.remainingLeaveDays < 5 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
                                }`}>
                              ...
                              <td className="py-4 px-6">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                                  emp.isActive !== false ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                                }`}>
          ...
          {activeTab === 'payroll' && (user?.role === 'admin' || user?.role === 'manager') && (
            <div className="space-y-8 animate-in zoom-in-95 duration-500">
              <div className="bg-white rounded-2xl md:rounded-3xl shadow-xl border border-slate-200 p-6 md:p-12 text-center space-y-6 md:space-y-8">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 mx-auto shadow-inner">
                  <Receipt size={32} />
                </div>
                <div>
                  <h2 className="text-xl md:text-2xl font-black text-slate-800">Execute Monthly Ledger</h2>
                  <p className="text-slate-500 text-sm md:text-base">
                    {new Date().toLocaleString('default', { month: 'long', year: 'numeric' })}
                  </p>
                </div>
                <button 
                  onClick={processPayrollRun} 
                  className="custom-theme-bg text-white px-8 py-4 md:px-12 md:py-5 rounded-2xl font-black text-sm md:text-lg shadow-2xl transition-all uppercase w-full md:w-auto"
                >
                  Process Payroll
                </button>
              </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                  ...
                  <div className="flex overflow-x-auto pb-2 w-full md:w-auto gap-2 scrollbar-hide">
                    <select 
                      ...
                      className="bg-slate-50 text-[10px] font-black uppercase p-2 rounded-lg border border-slate-100 min-w-[100px] shrink-0"
                    >
                    ...
                    <select 
                      ...
                      className="bg-slate-50 text-[10px] font-black uppercase p-2 rounded-lg border border-slate-100 min-w-[80px] shrink-0"
                    >
                      ...
                      {filteredPayroll.length > 0 ? (
                        filteredPayroll.map(record => {
                          ...
                          return (
                            <tr 
                              key={record.id} 
                              className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-all group"
                            >
                              ...
                              <td className="py-4 px-6 text-center text-[10px] font-bold text-slate-600">
                                {monthName} {record.year}
                              </td>
                              <td className="py-4 px-6 text-right text-xs font-black custom-theme-text">
                              ...
                              <td className="py-4 px-6 text-right">
                                <button 
                                  ...
                                  className="p-2 bg-slate-50 rounded-lg group-hover:bg-blue-500 group-hover:text-white transition-all"
                                >
                                  <Eye size={14} />
                                </button>
                              </td>
            ...
            <div className="space-y-6 md:space-y-10 animate-in fade-in duration-500">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={handlePrintP9Form}
                    className="flex items-center justify-center gap-2 bg-indigo-600 text-white px-4 py-3 rounded-xl font-bold shadow-xl text-xs no-print"
                  >
                    <FileText size={16} /> Print P9 Form
                  </button>
                </div>
              </div>
              ...
              <div className="print-content">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 print:grid-cols-1">
                  <div id="payslip-print-content" className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-8 print:shadow-none print:border-0">
                      ...
                      >
                        ...
                        {employees.map(e => (
                          <option key={e.id} value={e.id}>
                            {e.firstName} {e.lastName} ({e.payrollNumber})
                          </option>
                        ))}
                      </select>
                    )}
                  ...
                  <div id="p9-print-content" className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-8 print:shadow-none print:border-0">
                    <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2 print:text-base">
          ...
          {activeTab === 'settings' && user?.role === 'admin' && (
            <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-right-8 duration-500">
              <div className="mb-8">
                <h2 className="text-2xl md:text-3xl font-black text-slate-800 flex items-center gap-3">
                  <Palette className="custom-theme-text" /> Branding
                </h2>
              </div>
                ...
                <form onSubmit={handleSaveSettings} className="bg-white rounded-2xl md:rounded-[40px] shadow-sm border border-slate-100 p-6 md:p-10 space-y-8">
                  <div className="space-y-6">
                    <FormField 
                      ...
                      name="entityName" 
                      required 
                      defaultValue={brandSettings.entityName} 
                    />
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-4 p-4 border-2 border-dashed border-slate-100 rounded-2xl">
                        <div className="w-16 h-16 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 overflow-hidden">
                          {logoUrlInput ? (
                            <img src={logoUrlInput} className="w-full h-full object-contain" alt="Logo preview" />
                          ) : (
                        ...
                        <button 
                          ...
                          className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
                        >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 mt-2">
                          <input 
                            ...
                            className="h-12 w-12 p-1 rounded-xl bg-slate-50 border-2 border-slate-100 cursor-pointer" 
                          />
                          <input 
                            type="text" 
                            value={brandSettings.primaryColor} 
                            readOnly 
                            className="flex-1 bg-slate-50 border-2 border-slate-100 rounded-xl px-4 text-xs font-mono font-bold text-slate-400 uppercase" 
                          />
                        </div>
                      </div>
                    </div>
                    <div className="space-y-2">
                      ...
                      <textarea 
                        ...
                        rows={2} 
                        required 
                        defaultValue={brandSettings.address} 
                        className="w-full border-2 border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-700 outline-none focus:border-blue-500 resize-none text-sm" 
                      />
                    </div>
                  </div>
                  <button 
                    type="submit" 
                    className="w-full custom-theme-bg text-white font-black py-4 md:py-5 rounded-2xl shadow-xl transition-all hover:opacity-90 uppercase tracking-widest text-xs md:text-sm"
                  >
                    Deploy Brand
                  </button>
                </form>
                ...
                <div className="hidden lg:block space-y-4">
                  ...
                  <div className="bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl scale-95 origin-top p-6">
                    <div className="flex items-center gap-3 mb-6">
                      <div className="w-10 h-10 rounded-xl custom-theme-bg flex items-center justify-center text-white">
                        {logoUrlInput ? (
                          <img src={logoUrlInput} className="w-6 h-6 object-contain" alt="Preview logo" />
                        ) : (
                          <Building2 size={20} />
                        )}
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="w-full h-24 border-4 border-dashed border-slate-100 rounded-3xl flex items-center justify-center text-[10px] font-black uppercase text-slate-300">
                        Layout Block
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-slate-900 p-6 md:p-10 text-white flex justify-between items-center relative shrink-0">
              <div>
                <h3 className="text-xl md:text-3xl font-black">
                  {editingEmployee ? 'Update Profile' : 'Onboard User'}
                </h3>
              ...
              <button 
                onClick={() => { setShowAddEmployee(false); setEditingEmployee(null); }} 
            ...
            >
              <div className="md:col-span-2">
                <FormField 
                  ...
                  name="payrollNumber" 
                  required 
                  defaultValue={editingEmployee?.payrollNumber} 
              ...
              <FormField 
                label="Designation" 
                name="designation" 
                defaultValue={editingEmployee?.designation || editingEmployee?.position || 'Staff'} 
              ...
              <FormField 
                ...
                name="kraPin" 
                required 
                defaultValue={editingEmployee?.kraPin} 
              />
              <FormField 
                label="NSSF Reference" 
                name="nssfNumber" 
                required 
                defaultValue={editingEmployee?.nssfNumber} 
              />
              <FormField 
                label="SHA Identity" 
                name="nhifNumber" 
                required 
                defaultValue={editingEmployee?.nhifNumber} 
              />
              ...
              <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 p-4 md:p-6 bg-slate-50 rounded-2xl md:rounded-3xl border border-slate-100">
                ...
                <FormField 
                  ...
                  name="benefits" 
                  type="number" 
                  defaultValue={editingEmployee?.benefits} 
                />
              </div>
              ...
              <div className="md:col-span-2 grid grid-cols-2 gap-4 p-4 md:p-6 bg-blue-50 rounded-2xl md:rounded-3xl border border-blue-100">
                <FormField 
                  ...
                  name="totalLeaveDays" 
                  type="number" 
                  defaultValue={editingEmployee?.totalLeaveDays || 21} 
                />
                <FormField 
                  label="Remaining" 
                  name="remainingLeaveDays" 
                  type="number" 
                  defaultValue={editingEmployee?.remainingLeaveDays || 21} 
                />
              </div>
              ...
              <div className="md:col-span-2 flex flex-col md:flex-row justify-end gap-3 mt-4 pt-6 border-t border-slate-100">
                ...
                <button 
                  type="submit" 
                  className="w-full md:w-auto px-8 py-4 rounded-xl custom-theme-bg text-white font-black shadow-xl uppercase tracking-widest text-xs"
                >
                  {editingEmployee ? 'Update Ledger' : 'Commit Entry'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-t-[30px] md:rounded-[40px] shadow-2xl w-full max-w-2xl overflow-hidden animate-in slide-in-from-bottom md:zoom-in-95 flex flex-col h-[95vh] md:max-h-[90vh]">
            <div className="bg-slate-900 p-6 md:p-8 text-white flex justify-between items-center shrink-0">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 md:w-16 md:h-16 custom-theme-bg rounded-xl md:rounded-2xl flex items-center justify-center text-xl md:text-3xl font-black shrink-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-slate-400 text-[10px] font-bold mt-1">
                    Payroll No: {selectedEmployee.payrollNumber}
                  </div>
                  <div className="text-slate-300 text-[9px] font-bold mt-0.5">
                    {selectedEmployee.designation || selectedEmployee.position || 'Staff'}
                  </div>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-10 space-y-6 md:space-y-8 bg-slate-50/50">
              <section className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="bg-white p-5 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-2 md:space-y-3">
                    ...
                    <DetailRow 
                      label="Designation" 
                      value={selectedEmployee.designation || selectedEmployee.position || 'Not specified'} 
                    />
                    <DetailRow 
                      label="Payroll Number" 
                      value={selectedEmployee.payrollNumber} 
                    />
                    <DetailRow 
                      label="KRA PIN" 
                      value={selectedEmployee.kraPin} 
                    />
                    <DetailRow 
                      label="Email" 
                ...
                <div className="bg-white p-5 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-2 md:space-y-3">
                    <DetailRow 
                      label="Base Salary" 
                      value={`KES ${selectedEmployee.basicSalary.toLocaleString()}`} 
                    />
                    <DetailRow 
                      label="Benefits" 
                      value={`KES ${(selectedEmployee.benefits || 0).toLocaleString()}`} 
                    />
                    <DetailRow 
                      label="NSSF" 
              ...
              <section className="bg-white p-5 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-2 md:gap-6">
                  ...
                  <div className="text-center p-2 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl">
                    ...
                    <div className="text-base md:text-2xl font-black text-slate-800">
                      {selectedEmployee.totalLeaveDays - selectedEmployee.remainingLeaveDays}
                    </div>
                  </div>
                  <div className="text-center p-2 md:p-4 custom-theme-bg rounded-xl md:rounded-2xl text-white shadow-lg">
                    ...
                    <div className="text-base md:text-2xl font-black">
                      {selectedEmployee.remainingLeaveDays}
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-4">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 ml-1">
                  <BrainCircuit size={14} className="custom-theme-text" /> AI Intelligence
                </h4>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button 
                    onClick={() => handleGetTaxOptimization(selectedEmployee)} 
                    disabled={loadingTaxAdvice} 
                    className="bg-white border border-slate-200 text-slate-600 py-3 rounded-xl font-bold text-[8px] md:text-[10px] uppercase shadow-sm flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2"
                  >
                    {loadingTaxAdvice ? <Loader2 className="animate-spin" size={12} /> : <Lightbulb size={12} />} 
                    <span>Savings</span>
                  </button>
                  <button
                  ...
                  >
                    {loadingP9Breakdown ? <Loader2 className="animate-spin" size={12} /> : <FileSearch size={12} />} 
                    <span>Tax Audit</span>
                  </button>
                </div>
            ...
            <div className="p-4 md:p-8 border-t border-slate-100 bg-white grid grid-cols-2 md:flex gap-3 md:gap-4 shrink-0">
              <button 
                onClick={() => { setShowPayslipModal(true); setShowDetailModal(false); }} 
              ...
              <button 
                onClick={() => setShowDetailModal(false)} 
                className="col-span-2 custom-theme-bg text-white font-black py-4 rounded-xl md:rounded-2xl shadow-xl uppercase tracking-widest text-[9px] md:text-xs"
              >
                Close
              </button>
            </div>
...
const NavItem = ({ icon, label, active, onClick }) => (
  <button 
    onClick={onClick} 
    className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 font-bold text-sm tracking-tight ${
      active 
        ? 'custom-theme-bg text-white shadow-xl translate-x-1' 
...
const StatCard = ({ title, value, color, bgColor, icon }) => (
  <div className={`p-6 md:p-8 rounded-2xl md:rounded-[32px] border border-slate-100 shadow-sm transition-all hover:shadow-xl ${bgColor}`}>
    <div className="flex items-center justify-between mb-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">
      <div>{title}</div>
      <div className={`p-1.5 rounded-lg bg-white/50 ${color}`}>
        {icon}
      </div>
    </div>
    <div className={`text-xl md:text-3xl font-black ${color} truncate`}>
...
const DetailRow = ({ label, value }) => (
  <div className="flex justify-between items-center text-xs py-3 border-b border-slate-50 last:border-0">
    <span className="text-slate-400 font-bold uppercase tracking-widest text-[8px] md:text-[10px]">
      {label}
    </span>
    <span className="font-bold text-slate-800 truncate ml-2 max-w-[60%]">
      {value}
    </span>
  </div>
);

const FormField = ({ label, name, type = "text", required = false, placeholder = "", defaultValue }) => (
  <div className="space-y-2">
    ...
    <input 
      ...
      className="w-full border-2 border-slate-100 rounded-xl md:rounded-2xl px-5 py-3 md:py-4 bg-slate-50 focus:bg-white focus:custom-theme-border outline-none transition-all font-bold text-slate-700 text-sm" 
    />
  </div>
);
